-- Adds a preferred_foot to players (left/right/both), used for set-piece
-- taker selection and player cards. Weighted like real football roughly:
-- most players are right-footed, a meaningful minority left-footed, a
-- small slice comfortable on both.
...
-- Backfill existing players with the same weighted distribution new
-- players will get going forward, so old squads aren't all flatly 'right'.
UPDATE public.players
SET preferred_foot = (CASE
  WHEN random() < 0.72 THEN 'right'
  WHEN random() < 0.92 THEN 'left'
  ELSE 'both'
END)
WHERE preferred_foot = 'right';

CREATE OR REPLACE FUNCTION public._random_preferred_foot()
RETURNS TEXT AS $$
  SELECT CASE
    WHEN random() < 0.72 THEN 'right'
    WHEN random() < 0.92 THEN 'left'
...
-- generate_squad_for_club and _insert_generated_player both gain a
-- preferred_foot on INSERT (previously defaulted silently to 'right').
